...
def _keyed_metadata_condition(idx: int, col: FilterColumn, pred: Predicate, params: dict) -> str:
    ...
    "Metadata" means the tags on this trace, wherever the SDK attached them: the trace-level
    attribute lands in ``traces.metadata_map``, a span-level one in ``spans.metadata_map``, and
    the two key spaces are disjoint. A user filtering on a tag they can see should not have to
    know which scope set it, so a trace matches when the pair is present on its traces row OR on
    at least one of its spans. Unifying the two at query time rather than at ingest is what
    keeps this cheap: duplicating every trace-level key onto every span would inflate the
    largest table to answer a question the query layer answers directly.

    The trace half is an inline predicate on the already-scanned ``t`` row, so it costs no extra
    ...
    param names carry the predicate's index so two metadata predicates cannot collide.
